...
describe("competitive party normalization", function testSuite() {
  ...
  it("allows struggle only when no selectable move has PP", function testCase() {
    ...
    expect(isCompetitiveMoveSelectable(69)).toBe(true);
    expect(isCompetitiveMoveSelectable(97)).toBe(true);
    expect(canUseCompetitiveStruggle([{ moveId: 69, pp: 20 }])).toBe(false);
    expect(canUseCompetitiveStruggle([{ moveId: 97, pp: 30 }])).toBe(false);
