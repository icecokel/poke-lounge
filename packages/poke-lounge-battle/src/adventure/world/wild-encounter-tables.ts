import { MAX_POKEMON_LEVEL, MIN_POKEMON_LEVEL, type WildEncounterSlot } from "./wild-encounters";
...
function selectWildEncounterTable(
  ...
  const areaTable = areaId
    ? tableData.tables.find(function findItem(table) {
        return table.mapKeys.includes(mapKey) && table.areaIds?.includes(areaId);
      })
    : undefined;
  const mapTable = tableData.tables.find(function findItem(table) {
    return table.mapKeys.includes(mapKey) && !table.areaIds?.length;
  });
  const defaultTable = tableData.tables.find(function findItem(table) {
    return table.id === tableData.defaultTableId;
  });
...
export function normalizeWildEncounterTableData(
  ...
  const tables = data.tables
    .map(function mapItem(table) {
      return normalizeWildEncounterTable(table, version, pokemonRecords ?? []);
    })
    .filter(function filterItem(table): table is WildEncounterTable {
      return table !== null;
    });
...
function normalizeWildEncounterTable(
  ...
  return {
    ...
    ...(Array.isArray(data.areaIds)
      ? {
          areaIds: data.areaIds.filter(function filterItem(areaId): areaId is string {
            return typeof areaId === "string";
          }),
        }
      : {}),
    mapKeys: Array.isArray(data.mapKeys)
      ? data.mapKeys.filter(function filterItem(mapKey): mapKey is string {
          return typeof mapKey === "string";
        })
      : [],
    slots,
...
function normalizeExplicitEncounterSlots(data: unknown): WildEncounterSlot[] {
  ...
  return data
    ...
    .filter(function filterItem(slot): slot is WildEncounterSlot {
      return slot !== null;
    });
}
...
function expandEncounterSpeciesRanges(
  ...
  const ranges = data
    ...
    .filter(function filterItem(range): range is WildEncounterSpeciesRange {
      return range !== null;
    });
  ...
  const records = pokemonRecords.filter(function filterItem(record) {
    return ranges.some(function testItem(range) {
      return (
  ...
  const rareRecords = commonNationalDexIds
    ? records.filter(function filterItem(record) {
        return !commonNationalDexIds.has(record.nationalDexId);
      })
    : [];
  ...
  return records.map(function mapItem(record) {
    return {
...
function normalizeEncounterPokemonRecords(data: unknown): EncounterPokemonRecord[] | null {
  ...
  const records = Object.values(data.species)
    ...
    .filter(function filterItem(record): record is EncounterPokemonRecord {
      return record !== null;
    })
    .sort(function compareItems(left, right) {
      return left.nationalDexId - right.nationalDexId || left.speciesId - right.speciesId;
    });
