...
test("저장된 월드 위치와 방향이 현재 맵 범위 안이면 복원한다", function testCase() {
  assert.deepEqual(
...
test("다른 맵이나 범위 밖 위치와 잘못된 방향은 기본 스폰을 사용하도록 거부한다", function testCase() {
...
test("월드 위치는 싱글 진행에서만 저장해 경쟁 모드 이동과 분리한다", function testCase() {
