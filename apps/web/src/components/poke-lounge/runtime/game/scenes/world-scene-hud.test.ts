...
test("랭크와 점수 HUD는 현재 게임의 소수 점수를 표시한다", function testCase() {
  ...
  state.tournament.scoresByPlayerId = { [state.currentPlayerId]: 133.33, other: 200, tied: 133.33 };
  assert.deepEqual(getCurrentGameRankScore(state), { rank: 2, score: 133.33 });
  assert.match(formatRankScoreHud(getCurrentGameRankScore(state)), /133.33/);
});
