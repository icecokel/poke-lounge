import { createTournamentState } from "../tournament/tournament-state";
import {
...
export function createTournamentStartedAuthorityPayload({
  ...
  return normalizeTournamentRoomPayload("TOURNAMENT_STARTED", {
    ...
    participantIds: roster.participants.map(function mapItem(participant) {
      return participant.playerId;
    }),
    matchIds:
      tournament.currentRound?.matches.map(function mapItem(match) {
...
export function canApplyRoundScoreUpdatedAuthorityPayload(
  ...
  const expectedStanding = context.standings.find(function findItem(standing) {
    return standing.playerId === normalizedPayload.playerId;
  });
...
function findReadyTournamentMatch(
  ...
  return (
    session.tournament.currentRound?.matches.find(function findItem(match) {
      return match.matchId === normalizedMatchId && match.status === "ready";
    }) ?? null
  );
}

function isTournamentParticipant(session: TournamentSession, playerId: string): boolean {
  return session.tournament.participants.some(function testItem(participant) {
...
function standingsCoverTournamentParticipants(
  ...
  const expectedParticipantIds = session.tournament.participants.map(function mapItem(participant) {
    return participant.playerId;
  });
  const standingPlayerIds = standings.map(function mapItem(standing) {
...
function sameStandings(
  ...
  return left.every(function testItem(leftStanding) {
    return right.some(function testItem(rightStanding) {
      return (
...
function sameStringArray(
  ...
  return left.every(function testItem(value, index) {
...
function sameStringSet(left: ReadonlyArray<string>, right: ReadonlyArray<string>): boolean {
  ...
  return left.every(function testItem(value) {
