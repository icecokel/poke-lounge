...
import type { BattleSpriteRef } from "./battle-types";
...
export function getBattlePokemonAssets(speciesId: number): BattlePokemonAssetSet {
  ...
  const range = getBattlePokemonSpriteSheetRanges().find(function findItem(candidate) {
    return speciesId >= candidate.startSpeciesId && speciesId <= candidate.endSpeciesId;
  });
...
export function toBattlePokemonPreloadAssets(): BattlePokemonPreloadAsset[] {
  const preloadAssets = getBattlePokemonSpriteSheetRanges().flatMap(function mapItem(range) {
    return (["front", "back"] as const).map(function mapItem(side) {
      return {
