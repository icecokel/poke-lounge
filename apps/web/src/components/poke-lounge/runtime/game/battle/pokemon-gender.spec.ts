...
test("성비 경계값은 수컷·암컷·무성 포켓몬을 확정한다", function testCase() {
...
test("혼합 성비는 personal gender_ratio 임계값으로 성별을 생성한다", function testCase() {
  assert.equal(
    createPokemonGenderFromRatio(127, function callback() {
      return 0;
    }),
    "female",
  );
...
test("구버전 저장처럼 성비를 알 수 없으면 성별을 강제로 만들지 않는다", function testCase() {
