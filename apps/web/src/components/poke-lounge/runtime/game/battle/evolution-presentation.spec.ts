...
test("한국판 하트골드 ROM의 진화 시작·성공 문구를 그대로 조합한다", () => {
...
test("원작 진화 연출은 구형·신형 실루엣을 가속 교대하고 새 모습으로 끝난다", () => {
  const sampledFrames = [0.18, 0.36, 0.52, 0.64, 0.72, 0.77, 0.81].map(progress =>
    resolveRomEvolutionAnimationFrame(progress),
  );

  assert.ok(sampledFrames.every(frame => frame.stage === "morph"));
  assert.ok(sampledFrames.some(frame => frame.pokemon === "from"));
  assert.ok(sampledFrames.some(frame => frame.pokemon === "to"));
  assert.ok(sampledFrames.every(frame => frame.silhouetteAlpha === 1));
