import type { BattleAssetManifest, BattleAssetManifestEntry } from "./battle-types";
...
export async function loadBattleAssetManifest(
  ...
  return {
    ...manifest,
    byKey: new Map(
      manifest.assets.map(function mapItem(asset) {
...
function normalizeBattleAsset(value: unknown): BattleAssetManifestEntry | null {
  ...
  return {
    ...
    notes: Array.isArray(value.notes)
      ? value.notes.filter(function filterItem(note): note is string {
