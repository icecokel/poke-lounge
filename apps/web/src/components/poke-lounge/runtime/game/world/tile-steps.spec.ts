...
import { consumeCompletedTileSteps, createTileStepTracker } from "./tileSteps";

test("프레임 드롭으로 여러 타일을 지나도 모든 직교 이동을 반환한다", () => {
...
test("대각 이동은 같은 시점에 통과한 두 경계를 한 타일 이동으로 반환한다", () => {
  const tracker = createTileStepTracker({ x: 16, y: 16 });
