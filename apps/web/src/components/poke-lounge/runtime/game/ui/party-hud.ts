import { BATTLE_POKEMON_FRAME_SIZE, getBattlePokemonAssets } from "../battle/battle-pokemon-assets";
...
export function createPartyHudSlotViews({
  ...
  return Array.from({ length: PLAYER_PARTY_SLOT_COUNT }, function callback(_, slotIndex) {
    const slot = party.find(function findItem(partySlot) {
      return partySlot.slotIndex === slotIndex;
    });
