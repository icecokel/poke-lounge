...
`observe`와 입력 후 응답에는 **JPEG 이미지 바이트, SHA-256, 프레임 ID, 캡처 시각, 화면 타이머 문구, 보조 UI 텍스트**가 함께 있다. 이미지 없이 경로만 있는 응답, 잘린 JSON·한글 청크·해시 불일치는 성공으로 처리하지 않는다. 브라우저와 클라이언트는 외부 HTTP 터널이 아닌 현재 사용자의 로컬 Unix 소켓으로 연결된다.
...
**프레임에 시간 만료를 적용하지 않는다.** 20초 경과나 화면의 턴/라운드 타이머를 근거로 입력을 차단하지 않으며 `expiresAt`·`STALE_FRAME` 처리를 제거했다. 마지막으로 반환한 이미지의 ID·해시 확인과 중복 입력 방지는 유지한다. 화면이 바뀌었을 가능성이 있으면 `observe`로 다시 확인한 뒤 판단한다. 두 동시 입력을 뒤늦게 실행하는 대기열도 없다.

입력과 캡처의 결과는 분리된다. 입력 후 캡처가 실패해도 같은 요청 ID는 다시 실행되지 않는다. 타임아웃은 `uncertain`으로 남기며, 정상 여부를 새 화면으로 판정한다. 30초 넘는 관찰 공백은 `OPERATOR_GAP`과 `interrupted: true`로 기록하되 **입력을 막거나 브라우저를 종료하지 않는다.** 이 감사 기록은 이후 조회로 지워지지 않으며, 자동으로 테스트 PASS/FAIL을 판정하는 설정이 아니다. 어떤 명령도 우승·회차 완료·무버그 PASS를 자동으로 판정하지 않는다.

**무응답 시간에 따른 자동 종료는 없다.** 2분 이상 요청이 없어도 세션을 유지하며, `close` 또는 명시적 프로세스 종료 신호로 정리한다. **게임 시간은 멈추지 않는다.** 정상 완료 전에는 게임의 `방 나가기` UI를 직접 거쳐야 한다. 브라우저 종료를 서버의 방 정리 완료로 해석하지 않는다.
...
연결 도구가 응답을 자르면 출력 한도를 최소 600,000바이트로 지정한다. 이미 저장된 프레임의 바이트는 아래 명령으로 다시 받을 수 있다. 이것은 **원본 프레임 재전달**이지 새 화면 캡처가 아니며, 새로운 관찰이나 게임 시간 연장으로 취급하지 않는다.
...
`close` 성공이면 `SESSION_CLOSED`와 `audit.jsonl`의 `BROWSER_CLOSED`를 확인한다. 서버·DB·Redis를 이 도구가 임의로 종료하지 않는다. 자동 정리가 없으므로 작업 종료 시 만든 세션을 반드시 명시적으로 닫는다. 변경 전부터 실행 중인 드라이버에는 새 코드가 반영되지 않으므로 해당 세션을 닫고 새 이름으로 다시 연다. 이미지를 받지 못한 경우, 입력 실패, 브라우저 종료를 하나의 “Playwright 장애”로 합치지 않는다.

개별 입력·캡처·전송 요청이 멈췄는지 확인하는 작업 타임아웃은 유지한다. 이는 운영자가 다음 명령을 보내기까지의 시간 제한이나 무응답 세션 종료와 다르다. 게임의 턴 제한시간·AI 기본 행동·라운드 진행은 변경하지 않는다.
...
# 실제 125초 동안 요청을 보내지 않은 뒤 같은 CLI 세션의 입력 재개·종료 검증
PLAYER_BROWSER_VERIFY_IDLE=1 pnpm test:player-browser:integration
```
...
[플레이어 관점 반복 테스트·버그 수정 프롬프트](./playtest-prompt.md)를 새 작업에 사용한다. 우승 조건 없이 3라운드 정상 완주를 검사하며 연속 3회 PASS 또는 총 10회 시도에서 종료한다. 실제 조작 문제의 기록·수정·회귀 검증·커밋 기준을 포함하고, 푸시는 별도 요청 시 수행한다. 상세 실행 원장과 증거는 기존대로 Git에서 제외된 `docs/`·`output/`에 저장한다.
