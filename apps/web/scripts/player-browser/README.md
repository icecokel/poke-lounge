...
## 반복 플레이·버그 수정 작업 프롬프트

[플레이어 관점 반복 테스트·버그 수정 프롬프트](./playtest-prompt.md)를 새 작업에 사용한다. 유효 완주 5회, 회차별 포획 성공 3회·무작위 상점 구매 1회·종합 우승 목표, 버그 기록·수정·재검증·커밋·푸시 기준을 포함한다. 상세 실행 원장과 증거는 기존대로 Git에서 제외된 `docs/`·`output/`에 저장한다.
