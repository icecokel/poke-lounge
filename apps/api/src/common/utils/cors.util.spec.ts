...
describe('cors util', function testSuite() {
  it('기본 origin은 로컬 개발 웹만 포함한다', function testCase() {
  ...
  it('Vercel preview origin은 CORS_ORIGINS에 명시한 경우에만 허용한다', function testCase() {
  ...
  it('origin이 아닌 값이나 wildcard는 허용 목록에서 제외한다', function testCase() {
    expect(
      resolveCorsOrigins(
        [
          '*',
  ...
  it('Nest CORS 옵션은 검증된 origin, credentials, 요청 ID와 rate limit 헤더 노출을 사용한다', function testCase() {
