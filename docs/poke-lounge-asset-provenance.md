...
| Public gameplay data                  | `apps/web/public/game-data/pokemon-data.json`, `apps/web/public/game-data/item-data.json`, `apps/web/public/game-data/level-up-move-table.json`, `apps/web/public/game-data/wild-encounter-tables.json`, `apps/web/public/game-data/wild-battle-move-sets.json`, `apps/web/public/game-data/battle-pokemon-assets.json`, `apps/web/public/game-data/battle-screen-assets.json`, `apps/web/public/game-data/bootstrap.json`                                                                                                                                                   | Confirmed local extraction evidence; owner decision required            | The extractor generates the Pokémon, move, item, level-up, growth, sprite, and evolution files. Local and competitive battle runtime consume the generated records. They have no shipped provenance or license record.                                                                                                                                                                                                                                                                                                                                              |
...
`docs/poke-lounge-asset-provenance.json` records all 72 audited public files with local SHA-256 values. The audio manifest and eight source-derived audio rows are `blocked` until a release owner records a rights decision; the other 63 rows also remain `blocked`. `pnpm check:poke-lounge-provenance` validates the public-file coverage, hashes, source record, approval fields, and attribution fields. The command remains intentionally strict, but the default Web build only enforces it when `POKE_LOUNGE_PROVENANCE_STRICT=1` is set.
