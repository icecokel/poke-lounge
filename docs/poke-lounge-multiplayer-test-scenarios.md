...
멀티플레이와 개인 성장의 세부 판정은 규칙 인덱스와 연결된 하위 문서를 따른다. 6·7인 정원,
오류·재접속과 개인 성장 결과는 독립 시나리오이며 3인 한 사이클의 필수 단계가 아니다.
...
이 문서에서 **한 사이클**은 새 임시 비밀번호의 3인 room에서 방 생성·순차 참가부터 shared
world, 3개 게임 라운드, 최종 순위·우승자 수렴, 전원 UI leave와 room `closed`까지 한 번
진행하는 실행이다. **반복 검증**은 완전히 정리된 독립 한 사이클을 새 임시 비밀번호로 다시
수행하는 상위 절차다.
...
| `MP-LOBBY-015`  | P0/A          | AI 포함 참가자 6명에서 추가 시도     | 추가 버튼이 비활성화되고 서버도 정원 초과를 거부한다.                   |
...
| `MP-CAP-006` | P1/A          | `MP5` Socket을 끊고 59,999ms 동안 관찰             | 재접속 유예 중에는 자리를 유지해 다른 신규 사용자의 접속을 거부한다.     |
| `MP-CAP-007` | P1/A          | `MP5`가 60초 안에 같은 탭으로 재연결               | 만료를 취소하고 동일 참가자로 복원한다.                                  |
...
1. `MP1`~`MP6`이 같은 `PW_A`로 비공개 방에 순서대로 접속한다.
...
### 6.6 3인 shared world·3라운드 챔피언십 한 사이클
...
4. 관리자 승인 뒤 `MP2`, 이어서 `MP3`이 서로 다른 context에서 멀티플레이 탭을 선택하고
...
4. Mobile은 `필드 조작` deck이 열려 있으면 접근성 이름 `뒤로` control로 먼저 닫고
   ...
   유지 시간을 통과 조건으로 삼지 않는다. 변화가 없을 때만 같은 방향으로 한 번 더 입력하고 다른 두
...
1. 준비 종료 뒤 세 화면에서 seed 1=`MP1`의 부전승과 seed 2=`MP2` 대 seed 3=`MP3` 첫 대진이
   같은지 확인한다. 세 참가자 모두 server competitive Battle scene에 있고, `MP2`·`MP3`만 행동
   control을 사용하며 `MP1`은 입력 잠금 관전자여야 한다. 준비 중 열린 로컬 야생전은 남지 않는다.
2. 두 전투 참가자는 입력 watcher와 다음 turn 반복 루프까지 준비한 뒤 공유 관리자 채널에 정확한
   ...
   snapshot에서 `Poke Lounge 게임 화면` ref를 얻어 `focus`한 다음 `싸운다` Enter를
   입력한다. game surface `click`은 pointer confirm까지 발생시키므로 focus 용도로 사용하지 않는다.
   `move-select` 전환 뒤 fresh snapshot에서 game-surface ref를 다시 얻어 `focus`하고 첫 공격 기술 Enter를
   입력한다. Mobile은 화면에 표시된 `싸운다`, 첫 공격 기술을 차례로 touch한다. `ACTION-GO` 뒤 첫
...
4. 요청 자체가 없으면 현재 phase와 focus를 캡처하고 같은 UI 절차를 한 번만 다시 수행한다. 두
   ...
   보존하며 내부 API로 대신 제출하지 않는다. `MP1`은 같은 Battle scene을 입력 잠금 상태로
   관전하고 해당 대진 행동을 제출할 수 없어야 한다.
...
6. 로컬 `phase=ended`, `result` 존재와 화면 결과 control을 모두 확인한 시점을 UI terminal
   ...
   Desktop은 game surface focus 후 Enter, Mobile은 화면의 `다음`을 정확히 한 번 입력하고 scene·match가
   ...
   같은 결승 Battle scene의 입력 잠금 관전 상태로 전환해야 한다. 세 화면에서 같은 결승 대진을
   확인한다. 이
...
7. 결승 참가자들은 새 match의 `command`, turn 0과 입력 control을 확인하고 정확한
   ...
   관리자 승인 없이 연속 수행한다. 비참가자는 같은 Battle scene을 입력 잠금 상태로 관전한다.
...
각 실행 안에서 세 화면과 서버 결과가 일치하는지를 판정한다. 두 번 연속 세 runner가 문서 개정
...
아래 증적은 실행한 시나리오에 해당하는 항목만 필수다. 독립 시나리오의 증적을 3인 한 사이클에
억지로 포함하지 않는다.
...
3인 한 사이클은 아래 담당 플레이어가 checkpoint를 캡처한다. `전원` checkpoint는 세 화면에서
각각 남긴다.
...
| `C2-BYE`             | 전원   | seed 1 입력 잠금 관전, seed 2 대 3 활성 대진, 같은 Battle scene           |
...
| `C2-FIRST-CONFIRMED` | 전원   | 승자의 결승 battle, 패자·부전승자의 결승 관전, 관전자 action 없음         |
...
- 세 화면의 준비 시작·종료 시각
- 역할별 참가 순서·seed·부전승·대진 승패·게임 라운드별 terminal HP 점수
...
4. 방장은 혼자여도 파티 동기화와 ready를 마치면 시작할 수 있으며, 여러 명이면 전원이 ready일
   때만 시작할 수 있다.
...
19. 3인 한 사이클은 세 라운드의 대진·점수·최종 우승자가 세 화면과 Redis 권위 상태에서 같고,
...
| 5인 부전승·12대진         | `tournament-bracket.spec.ts`, `poke-lounge-five-player-tournament.spec.ts`                                                    | 없음                                             |
...
정리를 PASS했다. 이는 현재 working tree보다 이전 구현의 역사적 증적이므로 현재 3인 한 사이클의
