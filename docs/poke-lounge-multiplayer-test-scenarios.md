...
동작을 뜻한다. 공개 첫 화면은 솔로 플레이가 기본이며, 멀티플레이 사용자는 멀티플레이 탭을
선택한 뒤 닉네임과 임시 비밀번호를 제출한다. 사용자에게 별도의 방 만들기·방 들어가기 선택
화면을 노출하지 않는다.
...
| ID             | 우선순위/상태 | 절차                                   | 기대 결과                                                                         |
| -------------- | ------------- | -------------------------------------- | --------------------------------------------------------------------------------- |
| `MP-ENTRY-001` | P0/A          | 비로그인 진입 뒤 멀티플레이 탭 선택    | 처음에는 솔로 탭이 선택되고, 탭 선택 뒤 닉네임·임시 비밀번호·접속 CTA가 표시된다. |
| `MP-ENTRY-002` | P0/N          | 멀티플레이에서 닉네임을 비우고 접속    | 닉네임 필수 안내가 표시되고 입장 화면을 유지한다.                                 |
| `MP-ENTRY-003` | P0/N          | 닉네임만 입력하고 접속                 | 임시 비밀번호 필수 안내가 표시되고 입장 화면을 유지한다.                          |
| `MP-ENTRY-004` | P0/A          | 멀티플레이 pane의 공개 control 확인    | 방 코드·초대·생성/참가·로그인·경쟁전 설정이 없다.                                 |
...
3. 두 context가 멀티플레이 탭을 선택하고 서로 다른 닉네임과 같은 `PW_A`로 접속한다.
...
1. `MP1`~`MP6`이 멀티플레이 탭을 선택하고 같은 `PW_A`에 순서대로 접속한다.
...
1. `MP1`, `MP2`가 멀티플레이 탭에서 같은 `PW_A`에 접속해 각자의 파티를 준비한다.
